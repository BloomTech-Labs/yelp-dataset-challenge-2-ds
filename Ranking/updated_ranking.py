...
class Rankings(object):
    ...
    def get_star_estimate_bayesian_mean(self):
        """
        Computes the Bayesian mean from the prior and confidence.
        """
    ...
    def get_comparison_ids(self):
        ...
        # competitors_string_list = self.business_df.competitors.iloc[0]
        # comparison_ids = competitors_string_list.split(', ')
    ...
    def get_business_data(self):
        ...
        try:
            with get_session() as session:
                ...
                business_query = session.query(Business.review_count, \
                    Business.stars).filter(Business.business_id==self.business_id).all()

                # TODO add comparison to business_df
                # business_query = session.query(Business.review_count, \
                    # Business.stars, Viz2.competitors \
                    # ).join(Viz2).filter(Business.business_id==self.business_id).all()
